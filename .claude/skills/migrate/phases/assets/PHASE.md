...
1. **Scan.** Every mirrored `page` and `probe` body is read for `img src`, each
   ...
   from.
2. **Canonicalize and group.** Framer serves every asset from
   `framerusercontent.com`. Originals live at
   `framerusercontent.com/images/<id>.<ext>`; a responsive variant of the same
   image is the same URL with a `?scale-down-to=<width>` query string, and
   those variants are skipped during grouping (they are re-derivable from the
   original) — canonicalizing strips the query entirely. The same upload
   referenced from several pages becomes one record. Its `sources` list every
   place it was found, its `alt` is the most frequent non-empty `alt` seen, and
   `kind` becomes `video` if any reference hinted video.
