...
const LOGO_VARIANT_URL = `${LOGO_URL}?scale-down-to=512`;
const LOGO_STORE_PATH = "assets/0lkc1ii8jjl4rph5oy56h6z4pug.svg";
...
describe("createReplayLookup", () => {
  ...
  it("folds a framer srcset variant onto the same id as the stored canonical asset", async () => {
    await withProject(async (project) => {
      const lookup = await createReplayLookup(project);
