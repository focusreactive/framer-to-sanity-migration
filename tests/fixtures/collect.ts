...
const DETECTION_GROUPS = ['framer', 'negative'] as const;
...
const INVENTORY_PLATFORMS = ['framer'] as const;
...
async function crawlByPlatform(opts: {
  ...
  const { origin, sourceUrl, sitemapUrls, searchIndexUrl, store, maxPages, logger } = opts;
...
async function collectSnapshotFixture(args: SnapshotCollectArgs): Promise<void> {
  ...
  try {
    ...
    const { searchIndexUrl } = detect.platformHints;
