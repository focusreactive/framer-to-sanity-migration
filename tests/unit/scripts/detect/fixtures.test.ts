...
// disk, while still picking up any NEW framer/negative fixtures.
const CALIBRATION_GROUPS = new Set(["framer", "negative"]);
...
describe("detection fixtures — real detector over collected sites", () => {
  ...
  describe.each(fixtures)("$group/$name", ({ group, name, dir }) => {
    ...
    it(`verdict matches expected.json (${label})`, async () => {
      ...
      expect(
        ...
        `${label}: verdict ${result.verdict} != expected ${expected.verdict} `
          + `(framer score=${result.scores.framer.score} tier1=${result.scores.framer.hasTier1Strong})`,
    ...
    it(`hasTier1Strong flags are correct for the group (${label})`, async () => {
      ...
      if (group === "framer") {
        expect(result.scores.framer.hasTier1Strong, `${label}: expected a Tier-1 strong Framer signal`).toBe(true);
      } else {
        expect(
          result.scores.framer.hasTier1Strong,
          `${label}: negative fixture must have NO Tier-1 strong Framer signal`,
        ).toBe(false);
      }
    });
  });

  it("framer/pearlstudio exposes framerSiteId in platformHints", async () => {
    const fixture = fixtures.find((entry) => entry.group === "framer" && entry.name === "pearlstudio");
    expect(fixture, "framer/pearlstudio fixture must exist").toBeDefined();

    const result = detectPlatform(await loadDetectionFixture(fixture!.dir));
    expect(result.platformHints.framerSiteId).toBeTruthy();
  });
});
