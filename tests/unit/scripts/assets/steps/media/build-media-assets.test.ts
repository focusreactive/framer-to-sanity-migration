...
async function build(config: FakeStoreConfig): Promise<MediaAssetsData> {
  return buildMediaAssets({
    ...
    normalizer: framerMediaNormalizer,
    logger: nullLogger,
  });
}
...
describe("buildMediaAssets", () => {
  ...
  it("keeps only the original and skips scale-down-to responsive variants", async () => {
    const original = cdn(`/${SITE}/${A1}.jpg`);
    const html = `<img src="${original}?width=1024&height=683" srcset="${original}?scale-down-to=512&width=1024&height=683 512w, ${original}?scale-down-to=1024&width=1024&height=683 1024w">`;
    const data = await build({
  ...
  it("folds a bare original and its intrinsic-size query onto one canonical asset", async () => {
    const canonical = cdn(`/${SITE}/${A1}.jpg`);
    const data = await build({
      pages: [
        {
          url: "https://site.example/",
          html: `<img src="${canonical}?width=1024&height=683"><img src="${canonical}">`,
        },
      ],
  ...
  it("marks the second asset with identical bytes as an alias of the first", async () => {
    const u1 = cdn(`/${SITE}/${A1}.jpg`);
    const u2 = cdn(`/${SITE}/${A2}.jpg`);
