...
const FIXTURE_DIR = join(import.meta.dirname, "../../../fixtures/inventory/framer/pearlstudio");
...
describe("snapshotSite (fixture inventory -> pages.json -> snapshot)", () => {
  it("renders every crawled page with full store coverage, downloads the .mjs dep, and skips the media dep", async () => {
    ...
    try {
      ...
      // would (same pattern as tests/unit/adapters/framer/inventory/fixtures.test.ts).
      ...
      const { pages: classifiedPages } = await crawlFramer({
        origin,
